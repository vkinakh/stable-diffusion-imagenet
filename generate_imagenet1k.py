...
def main():
    ...
    out_dir = Path('./imagenet1k')
    out_dir.mkdir(exist_ok=True, parents=True)

    with open('imagenet1k_idx_to_class.txt', 'r') as f:
        idx_to_class = eval(f.read())

    for cls_id, lemma in tqdm(idx_to_class.items()):
        ...
        for img_idx in range(n):
            ...
            with autocast('cuda'):
                ...
                image = out["sample"][0]
                out_path = curr_dir / f'{img_idx:03d}_{seed}.png'
                image.save(out_path)
